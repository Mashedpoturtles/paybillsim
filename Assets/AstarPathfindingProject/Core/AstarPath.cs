...
public class AstarPath : MonoBehaviour
    ...
    public static bool IsUsingMultithreading
        ...
        get
            ...
            if ( threads != null && threads.Length > 0 )
                return true;
            else if ( threads != null && threads.Length == 0 && _threadEnumerator != null )
                return false;
                ...
                throw new System.Exception ( "Not 'using threading' and not 'not using threading'... Are you sure pathfinding is set up correctly?\nIf scripts are reloaded in unity editor during play this could happen.\n" +
                    ( threads != null ? "" + threads.Length : "NULL" ) + " " + ( _threadEnumerator != null ) );
            else
                return false;
            }
        }
	 ...
	 */
    private static IEnumerator<float> _threadEnumerator;
...
#line default
    ...
    private void Update ( )
        ...
        //Process paths
        if ( _threadEnumerator != null )
            {
            try
                {
                _threadEnumerator.MoveNext ( );
                }
            catch ( System.Exception e )
                {
                //This will kill pathfinding
                _threadEnumerator = null;
	 ...
	 */
    IEnumerator<float> _DelayedGraphUpdate ( )
        {
        graphUpdateRoutineRunning = true;

        yield return Timing.WaitForSeconds ( graphUpdateBatchingInterval - ( Time.realtimeSinceStartup - lastGraphUpdate ) );
        QueueGraphUpdates ( );
    ...
    public void UpdateGraphs ( GraphUpdateObject ob, float t )
        {
        Timing.RunCoroutine ( _UpdateGraphsInteral ( ob, t ) );
        }

    /** Update all graphs using the GraphUpdateObject after \a t seconds */
    IEnumerator<float> _UpdateGraphsInteral ( GraphUpdateObject ob, float t )
        {
    ...
    public void UpdateGraphs ( GraphUpdateObject ob )
        ...
        if ( batchGraphUpdates && Time.realtimeSinceStartup - lastGraphUpdate < graphUpdateBatchingInterval )
            ...
            if ( !graphUpdateRoutineRunning )
                {
                Timing.RunCoroutine ( _DelayedGraphUpdate ( ) );
                }
            }
        else {
    ...
    void Awake ( )
        ...
        if ( numThreads == 0 )
            {
            _threadEnumerator = _CalculatePaths ( threadInfos [ 0 ] );
            }
        else {
            _threadEnumerator = null;
            }
	 ...
	 * \see StartPath
	 */
    private static IEnumerator<float> _CalculatePaths ( System.Object _threadInfo )
        {
        PathThreadInfo threadInfo;
